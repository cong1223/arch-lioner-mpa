...
const create = <T, R>({
  ...
  const token: string = Taro.getStorageSync(StorageKeys.accessToken);
  const enterpriseId = Taro.getStorageSync(StorageKeys.user) && (JSON.parse(Taro.getStorageSync('user')) as IUser).curEnterpriseId;
